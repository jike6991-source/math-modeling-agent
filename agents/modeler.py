...
_SYSTEM_PROMPT: str = (
    ...
    "   (d) 纯数据分析的图表（如需求热力图、数据分布图）应在求解之前就生成，"
    "因为它们不依赖求解结果，这样即使求解超时也至少有数据分析图可用。\n"
    "   (c) 示例模板：\n"
    "       try:\n"
    "           plt.figure(...)\n"
    "           # ... 画图逻辑 ...\n"
    ...
    "10. MILP/ILP 建模中严禁为每个个体（工人/车辆/人员）创建独立的二进制决策变量，"
    "这会导致变量数与个体数量成正比爆炸增长（如 100人×10天×40模式=40000个Binary），CBC无法求解。"
    "正确做法是使用聚合整数变量：\n"
    "   - 错误：x[(i,d,m)] = Binary 表示工人i第d天是否选模式m → 变量数 = O(人数×天数×模式数)\n"
    "   - 正确：w[(d,m)] = Integer 表示第d天选模式m的工人人数 → 变量数 = O(天数×模式数)\n"
    "   - 人数 N 作为一个 Integer 变量出现在目标函数或约束中，不展开为逐人的Binary\n"
    "   - 典型约束写法：lpSum(w[(d,m)] for m in modes) <= N（每天总人数不超过N）\n"
    "   - 需求覆盖：lpSum(w[(d,m)] * cover[m][h][g] for m in modes) >= demand[d][h][g]\n"
    "11. _results.pkl 必须增量写入：每个子问题求解完成后立即更新并覆盖写入 _results.pkl，"
    "不要等所有子问题都解完再一次性写入。这样即使后续子问题超时被杀，前面的结果和图表仍然在磁盘上。\n"
    "   模板：\n"
    "       _results = {'demand': demand}  # 数据先存\n"
    "       with open('_results.pkl','wb') as f: pickle.dump(_results, f)\n"
    "       # ... 问题1求解 + 图表生成 ...\n"
    "       _results['problem1'] = {结果}\n"
    "       with open('_results.pkl','wb') as f: pickle.dump(_results, f)\n"
    "       # ... 问题2求解 + 图表生成 ...\n"
    "       _results['problem2'] = {结果}\n"
    "       with open('_results.pkl','wb') as f: pickle.dump(_results, f)\n"
